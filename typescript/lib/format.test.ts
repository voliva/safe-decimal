...
import { Rounding } from "./format";

describe("toNumber", () => {
  it("gives the result of the fraction as a number", () => {
    expect(new SafeDecimal({ n: 1, d: 5 }).toNumber()).toBe(0.2);
  });
});

describe("toFractionString", () => {
  it("gives the result as a fraction of two numbers", () => {
    expect(new SafeDecimal({ n: 1, d: 5.25 }).toFractionString()).toBe(
      "1/5.25"
    );
  });
});
...
describe("toString", () => {
  describe("serializes to strings", () => {
    it("defaults to base 10 strings", () => {
      expect(new SafeDecimal(98.76).toString()).toBe("98.76");
    });

    it("defaults to 16 decimals", () => {
      expect(new SafeDecimal(10).div(3)!.toString()).toBe("3.3333333333333333");
    });

    it("defaults to rounding half-ceil", () => {
      expect(new SafeDecimal("1.5").toString({ maxDecimals: 0 })).toBe("2");
    });

    it("writes numbers in base 16", () => {
      expect(new SafeDecimal(12.125).toString({ radix: 16 })).toBe("c.2");
    });

    it("writes numbers in base 2", () => {
      expect(new SafeDecimal(12.125).toString({ radix: 2 })).toBe("1100.001");
    });
  });

  describe("rounding modes", () => {
    function roundNum(input: string, rounding: Rounding) {
      return new SafeDecimal(input).toString({
        maxDecimals: 2,
        rounding,
      });
    }
